...
import { clearJsonlScanCache, newestJsonlFile, readJsonlTail, scanJsonlTree } from './jsonl-scan.ts';
...
describe('scanJsonlTree', () => {
  ...
  it('parses only the bytes an agent appended since the last scan', async () => {
    ...
    const first = await scanJsonlTree(dir, { cutoffMs: CUTOFF, parse });
    expect(first).toHaveLength(2);
    expect(parsed).toBe(2);

    appendFileSync(path, line(NOW, 3));
    const second = await scanJsonlTree(dir, { cutoffMs: CUTOFF, parse });
  ...
  it('waits for a half-written tail to finish instead of dropping it', async () => {
    ...
    expect(await scanJsonlTree(dir, { cutoffMs: CUTOFF, parse })).toHaveLength(1);
  ...
  it('never opens a file untouched since the cutoff', async () => {
    ...
    expect(await scanJsonlTree(dir, { cutoffMs: CUTOFF, parse })).toEqual([]);
  ...
  it('is empty for a directory that does not exist', async () => {
    expect(await scanJsonlTree(join(dir, 'nope'), { cutoffMs: CUTOFF, parse })).toEqual(
      [],
    );
  });
...
describe('newestJsonlFile / readJsonlTail', () => {
  it('finds the most recently written session', async () => {
    ...
    expect(await newestJsonlFile(dir, CUTOFF)).toBe(join(dir, 'new.jsonl'));
  });
